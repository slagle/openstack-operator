...
func main() {
	...
	if err = (&corecontrollers.OpenStackDataPlaneNodeReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&corecontrollers.OpenStackDataPlaneNodeTemplateReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "OpenStackDataPlaneNodeTemplate")
		os.Exit(1)
	}
